   ...
   <?php if ($_THEME['banner_limit'] > 0): ?>
    ...
    <?php if ($_THEME['weibo_limit'] > 0 AND count($weibo) > 3): ?>
        ...
        <script type="text/javascript">
            $(function() {
                var scrtime;
                var $scrollWeibo = $('#scrollWeibo');
                $scrollWeibo.hover(function() {
                    clearInterval(scrtime);
                }, function() {
                    scrtime = setInterval(function() {
                        var $ul = $scrollWeibo.find('ul');
                        var liHeight = $ul.find("li:last").height();
                        $ul.animate({marginTop: liHeight + 20 + "px"}, 700, function() {
...
<div id="main_right">
    ...
    <?php if ($_ID == 1 AND $_AA['name'] == '杭州浙江大学校友会'): ?>

        <?php if(isset($clubs)):?>

        <?php if($sidebar_rec):?>
        <p class="column_tt">互动平台</p>
        <div class="aa_block">
            <?php foreach ($sidebar_rec as $lg): ?>
                <div class="ubox" style="width:60px;margin:3px">
                    <a href="<?= $lg['url'] ?>"  title="<?= $lg['title'] ?>" target="_blank" > <img src="<?= $lg['filename'] ?>" class="home_club_logo"> </a>
<p class="face_name"><a href="<?= $lg['url'] ?>" target="_blank"><?= $lg['title'] ?></a></p>
                </div>
            <?php endforeach; ?>
            <div class='clear'></div>
        </div>
        ...
        <p class="column_tt">热门俱乐部</p>
        <div class="aa_block">
            <?php foreach ($clubs as $c): ?>
                <div class="ubox" style="width:60px;margin:3px">
                    <a href="<?= URL::site('club_home?id=' . $c['id']) ?>" title="<?= $c['name'] ?>" target="_blank"> <img src="<?=$c['logo_path']?>" class="home_club_logo"> </a>
<p class="face_name"><a href="<?= URL::site('club_home?id=' . $c['id']) ?>"><?= str_replace('俱乐部','',$c['name']) ?></a></p>
                </div>
            <?php endforeach; ?>
            <div class='clear'></div>
        </div>
