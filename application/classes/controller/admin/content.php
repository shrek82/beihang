...
class Controller_Admin_Content extends Layout_Admin {
    ...
    function action_index() {
        ...
        $content = Doctrine_Query::create()
                ->select('c.id,c.title,c.create_at,c.is_system,c.is_recommended,c.update_at,c.redirect,t.name as cname')
                ->from('Content c')
    ...
    function action_recommended() {
        ...
        $news = Doctrine_Query::create()
                ...
                ->where('id = ?', $cid)
                ->fetchOne();
        if ($news['is_recommended'] == TRUE) {
            $news['is_recommended'] = FALSE;
        } else {
            $news['is_recommended'] = TRUE;
        }
        $news->save();
    }
