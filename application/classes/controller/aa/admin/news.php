...
class Controller_Aa_Admin_News extends Layout_Aa {
    ...
    function before() {
        ...
        $actions = array(
            'aa_admin_news/index' => '新闻列表',
            'aa_admin_news/form' => '添加新闻',
            'aa_admin_news/category' => '新闻分类',
        );
    ...
    function action_index() {
        $news = Doctrine_Query::create()
                ...
                ->addSelect('c.aa_id,c.name AS category_name')
                ->from('News n')
            ...
            if (!$valid->check()) {
                ...
                if ($view['cid'] == 0) {
                    ...
                    $cur_cat->removeDqlQueryPart('where')
                            ->where('aa_id = ? AND name = ?', array($this->_id, $post['name']));
                    $exist = $cur_cat->count();
                    if ($exist > 0) {
    ...
    function action_del() {
        ...
        $cid = Arr::get($_GET, 'cid');
        $news = Doctrine_Query::create()
                ->select('n.*,c.aa_id AS aa_id')
                ->from('News n')
                ->leftJoin('n.NewsCategory c')
                ->where('n.id =?', $cid)
                ->fetchOne();

        if ($news['aa_id'] == $this->_id) {
            $news->delete();
            $del = Doctrine_Query::create()
