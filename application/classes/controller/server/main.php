...
class Controller_Main extends Layout_Main {
    ...
    function action_index() {
        ...
        //推荐校友组织
        $view['aa'] = $this->_cache->get('home_aa2');
        if (!$view['aa']) {
            $view['aa'] = Db_Aa::topAa('地方校友会', 15);
            $this->_cache->set('home_aa2', $view['aa'], 84600);
        }
        ...
        $view['news_special'] = DB::select()
               ...
               ->limit(1)
                ...
                ->execute()
                ->as_array();
        if($view['news_special']){
            $view['news_special']=$view['news_special'][0];
        }
        ...
        if ($view['news_special']) {
            ...
            $view['top_news'] = Doctrine_Query::create()
                    ->select('n.id,n.title,n.intro,n.small_img_path')
                    ->from('News n')
        ...
        //显示新闻列表
            ...
            //头条新闻
            $view['top_news'] = DB::select(DB::expr('n.id,n.title,n.intro'))
                    ->from(array('news', 'n'))
                    ->where('n.is_draft', '=', 0)
                    ->where('n.is_top', '=', 1)
                    ->order_by('n.id', 'DESC')
           ...
           $view['top_news'] =$view['top_news']?$view['top_news'][0]:false;
            ...
            //没有头条新闻显示17条
            $news_size = $view['top_news'] ? 15 : 17;

            //更多新闻列表
            //$view['news_list'] = $this->_cache->get('home_fixed_news');
            //if (!$view['news_list']) {
                $view['news_list'] = DB::query(Database::SELECT, 'SELECT n.id,n.title,n.create_at, n.title_color,n.is_pic,n.is_fixed FROM `news` `n` LEFT JOIN `sys_filter` `s` ON `n`.`id` = `s`.`news_id` WHERE (`n`.`is_draft` = 0 AND `s`.`news_id` > 0 AND `n`.`is_top` = 0) ORDER BY `n`.`create_at` DESC LIMIT 15')->execute()->as_array();
            //    $this->_cache->set('home_fixed_news', $view['news_list'], 86400);
            //}
        }
        //北航校友
       ...
       if (!$view['units']) {
        ...
        $view['hot_topic'] = $this->_cache->get('home_hot_topicsss');
        ...
        if (!$view['hot_topic']) {
            ...
            $view['hot_topic'] = DB::query(Database::SELECT, $hot_topics_sql)
                    ->execute()
                    ->as_array();
            $this->_cache->set('home_hot_topicsss', $view['hot_topic'], 3600);
       }
        ...
        //if ($view['statistics']) {
            $view['statistics'] =DB::select(DB::expr('id,project,donate_at,donor,speciality,amount'))
                    ...
                    ->execute()
                    ->as_array();
           // $this->_cache->set('home_statistics', $view['statistics'], 48000);
       //}
