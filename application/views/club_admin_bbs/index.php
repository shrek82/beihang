...
<table id="bench_list" width="100%"  border="0"  cellpadding="0" cellspacing="0">
    ...
    <tr class="<?=(($key+1)%2)==0?'even_tr':'odd_tr';?>">
        <td style="padding:8px 2px;width:600px">
        <span style="color:#999">[<?=$subject[$u['subject']]?>]</span>&nbsp;<a target="_blank" href="<?= URL::site('bbs/view'.$u['type'].'?id='.$u['id']) ?>" <?= $u['is_club_fixed'] ? 'style="color:#f00"':'' ?>><?=Text::limit_chars($u['title'],35,'...'); ?></a>			<?php if ($u['is_good']): ?>
        ...
        <td class="center"><input type="checkbox" onclick="setBool(<?= $u['id'] ?>,'is_club_fixed')" <?= $u['is_club_fixed'] ? 'checked':'' ?> /></td>
...
function setBool(id,field){
    new Request({
        type:'post',
        url:'<?= URL::site('club_admin_bbs/set').URL::query() ?>',
