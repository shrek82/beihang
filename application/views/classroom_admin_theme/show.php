<!-- classroom_admin_base/show:_body -->
<div id="admin950">
 ...
 <br><br>
    <form action="/classroom_admin_theme/saveshow?id=<?= $_ID ?>" method="post" id="show_form">
        <table>
            <tr>
                <td style="width:90px; ">图片</td>
                <td>
                    <div id="uploading" style="display:none; color:#3993E0;width:600px; height:30px;"><img src="/static/images/loading4.gif"  hspace="4" align="absmiddle"  />正在上传中，请稍候...</div><input type="hidden" name="filename" id="filepath" value="<?= $show['filename'] ?>" /><iframe  id="upfileframe" name="upfileframe" scrolling="no" style="width:600px; height:30px; display:inline" frameborder="0" src="<?= URL::site('upload/frame?msg=banner:676*160px') ?>"></iframe>
...
<script type="text/javascript">
    ...
    function del(id){
        new candyConfirm({
            message:'确定要删除该图片展示吗？',
            url: '<?= URL::base() ?>classroom_admin_theme/delshow?id=<?= $_ID ?>&show_id='+id,
            removeDom:'s_'+id
        }).open();
    }
    ...
    function setDisplay(id){
        new Request({
            url: '<?= URL::base() ?>classroom_admin_theme/setHidden?id=<?= $_ID ?>&show_id='+id,
            type:'post'
        }).send();
    }
