                ...
                <tr class="<?= (($key + 1) % 2) == 0 ? 'even_tr' : 'odd_tr'; ?>">
                    ...
                    <td style="height:100px;line-height: 1.6em">
                        <strong style="font-size: 1.1em">
                            <a href="<?= Db_Event::getLink($e['id'],$e['aa_id'],$e['club_id'])?>" style="font-weight:bold;font-size:14px;<?= $e['is_fixed'] ? 'color:#f30':'';?>"><?=Text::limit_chars($e['title'],25)?></a>
                        </strong><br />
                        发起：<?= $e['realname'] ?><br>
                        地址：<?= $e['address'] ?><br>
                        发布：<?= $e['publish_at'] ?>
                    ...
                    <td class="center">
                        <input type="checkbox" onclick="setBool(<?= $e['id'] ?>,'is_fixed')" <?= $e['is_fixed'] == true ? 'checked' : '' ?> />
                    </td>
                    <td class="center">
...
<script type="text/javascript">
    function setBool(cid,field){
            new Request({
                type:'post',
                url: '<?= URL::site('aa_admin_event/set?id=' . $_ID) ?>',
