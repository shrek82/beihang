<!-- club_admin_base/info:_body -->
<div id="admin950">
    <div style="text-align: right; margin-bottom: 10px"><input class="button_blue" value="+添加新介绍" type="button"  onclick="window.location.href='<?= URL::site('club_admin_base/form?id=' . $_ID) ?>'"></div>
    ...
    <table id="bench_list" width="100%"  border="0"  cellpadding="0" cellspacing="0" style="margin-bottom: 60px">
        ...
        <?php foreach ($infos as $key => $inf): ?>
            <tr class="<?= (($key + 1) % 2) == 0 ? 'even_tr' : 'odd_tr'; ?>" id="info_<?= $inf['id'] ?>">
                <td style="padding:5px"><a href="<?= URL::site('club_admin_base/form?id=' . $_ID . '&cid=' . $inf['id']) ?>" title="点击进入编辑页面"><?= $inf['title'] ?></a></td>
    ...
    <script type="text/javascript">
        function del(cid){
            new candyConfirm({
                ...
                url: '<?= URL::site('club_admin_base/del?id=' . $_ID . '&cid=') ?>'+cid,
                callback:function(){
                    window.location.href='<?= URL::site('club_admin_base/info?id=' . $_ID) ?>';
                }
            }).open();
        }
        ...
        function setBool(cid,field){
            new Request({
                type:'post',
                url: '<?= URL::site('club_admin_base/set?id=' . $_ID) ?>',
