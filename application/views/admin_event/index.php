<!-- admin_event/index:_body -->
<?php
$etype = Kohana::config('icon.etype');
?>
<table width="100%" border="0" cellpadding="0" cellspacing="1" class="admin_table">
    <tr>
    ...
    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="margin-top:0px " class="admin_table">
        <tr>
    ...
    <tr>
        <td colspan="8" style="background-color:#fff;padding:10px; text-align: left; color: #999">没有任何活动。</td>
    </tr>
    <?php endif; ?>
    
        <?php foreach ($events as $key => $e): ?>
            <tr style="border-bottom:1px dotted #ccc;" class="<?php if (($key) % 2 == 0) {
            echo'even_tr';
        } ?>">
    ...
    <script type="text/javascript">
        function setEventBool(id, field){
            new Request({
                url: '<?= URL::site('admin_event/set') . URL::query() ?>',
