...
<div id="main_left" style="width: 680px;">
    ...
    <div id="hot_link">
        ...
        <?php if ($host_event_tags): ?>
            <?php foreach ($host_event_tags AS $t): ?><a href="/aa_home/event?id=<?=$_ID?>&tag=<?= urlencode($t['name']) ?>"><?= $t['name'] ?></a><?php endforeach; ?>
        <?php else: ?>
        ...
        热门话题：
        <?php if ($hot_topics): ?>
            <?php foreach ($hot_topics AS $t): ?><a href="/weibo?id=<?= $_ID ?>&topic=<?= urlencode($t['topic']) ?>"><?= $t['topic'] ?></a><?php endforeach; ?>
        <?php else: ?>
            <span class="nodata">暂时还没有话题</span>
        <?php endif; ?>
        <?php endif;?>
...
<div id="main_right">
    ...
    <?php if ($_CONFIG->base['id'] == 'zuaa' AND $_ID == 1): ?>
        <p class="column_tt">官方微博</p>
        <div class="aa_block">
            <iframe width="136" height="24" frameborder="0" allowtransparency="true" marginwidth="0" marginheight="0" scrolling="no" border="0" src="http://widget.weibo.com/relationship/followbutton.php?language=zh_cn&width=136&height=24&uid=1972023231&style=2&btn=red&dpc=1"></iframe>
        </div>
    <?php endif; ?>
