...
<div id="main_left">
    <div style="padding: 5px 20px">
        <form action="/aa_home/album?id=<?=$_ID?>" style="text-align: right" method="POST">
    ...
    <div class="album_index" >
        <?php if (!$albums): ?>
            <div class="nodata">暂时还没有自建相册。</div>
        <?php else: ?>
...
<div id="main_right">
    ...
    <div class="aa_block">
        <ul class="aa_conlist">
            ...
            <li><a href="/aa_home/album?id=<?= $_ID ?>&list=new" <?= $list == 'new' ? 'class="cur"' : ''; ?>>最近上传</a></li>
            <li><a href="/aa_home/album?id=<?= $_ID ?>&list=event" <?= $list == 'event' ? 'class="cur"' : ''; ?>>活动相册</a></li>
            <li><a href="/aa_home/album?id=<?= $_ID ?>&list=club" <?= $list == 'club' ? 'class="cur"' : ''; ?>>俱乐部相册</a></li>
            <li><a href="/aa_home/album?id=<?= $_ID ?>&list=aa" <?= $list == 'aa' ? 'class="cur"' : ''; ?>>公共相册</a></li>
            <li><a href="/aa_home/album?id=<?= $_ID ?>&list=all" <?= $list == 'all' ? 'class="cur"' : ''; ?>>所有相册</a></li>
        </ul>
    </div>
</div>
