...
<div id="main_left">
    <div style="text-align: right;padding: 10px">
        <input type="button" onclick="location.href='<?= URL::site('event/form?aa=' . $_ID) ?>'" value="+发起活动" class="button_blue"  />
    </div>
            ...
            <?php foreach ($events AS $key => $e): ?>
                <tr class="<?= (($key + 1) % 2) == 0 ? 'even_tr' : 'odd_tr'; ?>">
                    ...
                    <td style=" line-height: 1.6em;padding:15px 0;width:430px">
                        <a href="/aa_home/eventview?id=<?= $_ID ?>&eid=<?= $e['id'] ?>" style="font-weight:bold;font-size:14px;<?= $e['is_fixed'] ? 'color:#f30' : ''; ?>"><?= Text::limit_chars($e['title'], 30) ?></a>
                        <br />
                        地点：<?= $e['address'] ?><br>
...
<div id="main_right">
    ...
    <div class="aa_block">
        <ul class="aa_conlist">
            <li><a href="<?= URL::site('aa_home/event?id=' . $_ID . '&list=week') ?>" <?= $list == 'week' ? 'class="cur"' : ''; ?>>未来7天</a></li>
            <li><a href="<?= URL::site('aa_home/event?id=' . $_ID . '&list=today') ?>" <?= $list == 'today' ? 'class="cur"' : ''; ?>>今天</a></li>
            <li><a href="<?= URL::site('aa_home/event?id=' . $_ID . '&list=weeken') ?>" <?= $list == 'weeken' ? 'class="cur"' : ''; ?>>周末</a></li>
            <li><a href="<?= URL::site('aa_home/event?id=' . $_ID) ?>" <?= !$list ? 'class="cur"' : ''; ?>>所有活动</a></li>
        </ul>
    </div>

    <p class="column_tt">热门标签</p>

    <div class="aa_block">
        <?php foreach ($tags as $tag): ?>
            <a href="<?= URL::site('aa_home/event?id=' . $_ID . '&tag=' . urlencode($tag['name'])) ?>" ><?= $tag['name'] ?></a>&nbsp;
        <?php endforeach; ?></div>
