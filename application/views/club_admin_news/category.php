<!-- club_admin_news/category:_body -->
<div id="admin950">
            ...
            <?php if (count($category) > 0): ?>
                <?php foreach ($category as $key => $c): ?>
                    <tr id="c_<?= $c['id'] ?>" class="<?=(($key+1)%2)==0?'even_tr':'odd_tr';?>">
                        ...
                        <td style="text-align:center"><a href="/club_admin_news/category?id=<?=$_ID?>&cid=<?=$c['id']?>">编辑</a></td>
...
<script type="text/javascript">
    ...
    function del(cid){
        var b = new Facebox({
            ...
            ok: function(){
                new Request({
                    url: '<?= URL::site('club_admin_news/delcategory?cid=') ?>'+cid+'&id='+<?=$_ID?>,
                    type: 'post',
