<form action="" method="post" enctype="multipart/form-data">
    <table class="admin_table" border="0" cellpadding="0" cellspacing="1" width="100%">
        ...
        <tr>
            <td colspan="2" style="padding:10px 20px"><textarea id="content" name="content" style="width: 100%;height:400px"><?= @$content['content'] ?></textarea></td>
        </tr>

        <tr>
            <td class="field">相关图片：</td>
            <td><input type="file" name="file"  /></td>
        </tr>
        <?php if($content['id']):?>
        <tr>
            <td class="field">已传附件 ：</td>
            <td><input type="hidden" name="img_path"  value="<?= $content['img_path'] ?>"  style="width:300px" class="input_text" />
            
                <?php if($content['img_path']):?>
                <img src="<?= $content['img_path'] ?>" style="margin: 10px" />
                <?php else:?>
                <span style="color: #ccc">未上传附件</span>
                <?php endif;?>
        ...
        <tr>
            ...
            <td ><input type="text" name="start_date" value="<?= $content['start_date'] ? $content['start_date'] : date('Y-m-d') ?>"  style="width:150px" class="input_text"/> - <input type="text" name="end_date" value="<?= $content['end_date'] ? $content['end_date'] : date('Y-m-d', strtotime('+30 day'));
                ; ?>"  style="width:150px" class="input_text"/>&nbsp;&nbsp;<span style="color:#999">例如： 2011-01-01-2011-02-01，两个日期都必须填写</span></td>
        </tr>
        <tr>
