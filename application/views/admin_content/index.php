...
<table class="admin_table" width="100%" border="0" cellpadding="0" cellspacing="1" >
    <tr >
        <td height="29" class="td_title" colspan="7"><b>内容管理</b>
    ...
    <?php else: ?>
        ...
        <?php foreach ($content as $key => $c) : ?>
            <tr  id="con_<?= $c['id'] ?>" class="<?php if (($key) % 2 == 0) {
            echo'even_tr';
        } ?>">
                ...
                <td class="center"><input type="checkbox" value="1" onclick="recommended(<?= $c['id'] ?>);" <?= $c['is_recommended'] ? 'checked':'' ?> /></td>
...
<script type="text/javascript">
    function recommended(cid) {
        new Request({
            url: '<?= URL::site('admin_content/recommended') . URL::query() ?>',
            type: 'post',
            data: 'cid=' + cid
        }).send();
    }
