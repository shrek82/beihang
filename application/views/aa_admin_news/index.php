        ...
        <tr id="news_<?=$n['id']?>" class="<?=(($key+1)%2)==0?'even_tr':'odd_tr';?>" >
          ...
          <td class="timestamp" style="text-align:center;color: #999"><span style="color:#008000"><?=$n['comments_num']?></span>/<span style="color:#666"><?=$n['hit']?></span></td>
            ...
            <td class="handler" style="text-align:center">
                <a href="<?= URL::site('aa_admin_news/form?id='.$_ID.'&news_id='.$n['id']) ?>">修改</a>
            </td>
            <td style="text-align:center">
...
<script type="text/javascript">
    function setFixed(nid){
        new Request({
            url: '<?= URL::site('aa_admin_news/fixed').URL::query() ?>',
            type: 'post',
            data: 'cid='+nid
        }).send();
    }
    ...
    function set_top(cid){
        new Request({
            url: '<?= URL::site('aa_admin_news/top').URL::query() ?>',
            type: 'post',
	    data: 'cid='+cid
        }).send();
    }
function del(cid){
    var b = new Facebox({
        ...
        ok: function(){
            new Request({
                url: '<?= URL::site('aa_admin_news/del?id='.$_ID.'&cid=') ?>'+cid,
                type: 'post',
                success: function(){
                    candyDel('news_'+cid);
                }
