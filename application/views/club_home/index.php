...
<div id="main_left" style="width: 680px;">
    ...
    <?php if ($_THEME['news_limit'] > 0): ?>
        <div class="big_title">
            <p class="name">新闻动态</p><p class="tool"><?php if (($_MEMBER && $_MEMBER['manager'])): ?><a href="<?= URL::site('club_admin_news') . URL::query() ?>" class=" link2">管理</a><?php else: ?><a href="<?= URL::site('club_home/news') . URL::query() ?>" class="link2">更多</a><?php endif; ?></p></div>
        <?php if (!$news): ?>
        ...
        <ul class="con_list2">
            <?php foreach ($news as $n): ?>
                <li style="padding:2px 0"><?= $n['category_name'] ?>&nbsp;<a href="<?= URL::site('club_home/newsDetail?id=' . $_ID . '&nid=' . $n['id']) ?>" target="_blank"  <?= $n['is_fixed'] ? 'style="color:#f30"' : ''; ?>><?= Text::limit_chars($n['title'], 42, '...'); ?></a>
    ...
    <div class="big_title">
        <p class="name">近期活动</p>
        <p class="tool"><a href="<?= URL::site('/event/form?aa=' . $_CLUB['aa_id'] . '&club=' . $_ID) ?>" class="postlink">发起</a> | <?php if (($_MEMBER && $_MEMBER['manager'])): ?><a href="<?= URL::site('aa_admin_event') . URL::query() ?>">管理</a><?php else: ?><a href="<?= URL::site('club_home/event') . URL::query() ?>">更多</a><?php endif; ?></div>
    <?php if (!$event): ?>
    ...
    <ul class="con_list2">
        <?php foreach ($event as $e): ?>
            <li style="padding:2px 0">
                <?= $e['type'] ? $e['type'] : '无分类'; ?>&nbsp;<a href="<?= Db_Event::getLink($e['id'], $e['aa_id'], $e['club_id']) ?>"  target="_blank" <?= $e['is_club_fixed'] ? 'style="color:#f30"' : ''; ?>><?= Text::limit_chars($e['title'], 35, '..') ?></a><?php if ($e['sign_num'] > 0): ?>&nbsp;&nbsp;(已有<font style="color:#1F9800"><?= $e['sign_num'] ?></font>人参与)<?php endif; ?>
                <span class="date">
    ...
    <div class="big_title"><p class="name">最新话题</p>
        <p class="tool"><a href="<?= URL::site('bbs/unitForm?aa_id=' . $_CLUB['aa_id'] . '&club_id=' . $_ID) ?>" class="postlink link2">发帖</a> |  <?php if (($_MEMBER && $_MEMBER['manager'])): ?><a href="<?= URL::site('bbs/list?aid=' . $_ID) ?>" class="link2">管理</a><?php else: ?><a href="<?= URL::site('bbs/list?aid=' . $_CLUB['aa_id'] . '&cid=' . $_ID) ?>" class="link2">更多</a><?php endif; ?></p></div>
    <?php if (!$units): ?>
            ...
            <?php foreach ($units as $un): ?>
                <tr>
                    <td style="height: 34px;">
                        <?php if ($un['is_fixed'] OR $un['is_club_fixed']): ?>
                    ...
                    <td>
                        <a href="<?= URL::site('bbs/viewPost?id=' . $un['id']) ?>" style="<?= $un['is_fixed'] || $un['is_club_fixed'] ? 'color:#f30' : ''; ?>" title="<?= $un['title'] ?>" target="_blank">
    ...
    <!--photo -->
    <div class="big_title"><p class="name">活动相册</p>
        <p class="tool"><a href="/club_home/album?id=<?= $_ID ?>">更多</a></p>
    </div>
...
<div id="main_right">
    <?= View::factory('club_global/userbox'); ?>

    <p class="column_tt">相关介绍</p>
    <div class="aa_block">
        <ul class="aa_conlist">
            <li><a href="<?= URL::site('club_home/info?id=' . $_ID) ?>"><?= $_CLUB['name'] ?>简介</a></li>
            <?php foreach ($infos as $inf): ?>
                <li><a href="<?= URL::site('club_home/info?id=' . $_ID . '&info_id=' . $inf['id']) ?>"><?= Text::limit_chars($inf['title'], 13, '..') ?></a></li>
            <?php endforeach; ?>
        </ul>
    </div>
