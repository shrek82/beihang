            ...
            <?php if (count($hot_classroom)): ?>
                <ul class="ranking">
                    <?php foreach ($hot_classroom as $c): ?>
                        <li><a href="<?= URL::site('classroom_home?id=' . $c['id']) ?>" target="_blank" title="<?= $c['start_year'] ?>级<?= $c['name'] ?>"><?= $c['name'] ?$c['name']:$c['start_year'].'级'.$c['speciality'] ?></a><span>(<?= $c['member_num'] ?>人)</span></li>
                    <?php endforeach; ?>
                </ul>
            <?php else: ?>
